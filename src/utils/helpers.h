...
// get center of lane in frenet coords
inline double getCenterOfLaneFrenet(int lane) {
    return 4 * lane + 2;
}

inline double getCarYaw(double ref_x_previous, double ref_x, double ref_y_previous, double ref_y) {
  return atan2(ref_y - ref_y_previous, ref_x - ref_x_previous);
}

inline double distanceBetween(vector<double> p1, vector<double> p2) {
  return distance(p1[0], p1[1], p2[0], p2[1]);
}

inline int deriveLane(double d) {
